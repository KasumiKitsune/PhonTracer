...
**Windows**
- 主套件安装版：运行发布页中的 `PhonTracer_Setup_Windows_x64.exe` 安装程序（自动注册 `.teproj` 与 `.ptwl` 文件关联）。
- 录音配套工具安装版：运行发布页中的 `phonrec_1.3.0_x64-setup.exe` 或 `phonrec_1.3.0_arm64-setup.exe`。
- 便携版：解压发布页中的 Windows 压缩包后直接运行 `PhonTracer.exe`、`Toolkit.exe` 或 `PhonRec.exe`。
- 系统架构支持：各组件提供针对 Windows x64 与 Windows ARM64 架构的专用打包版本。

**macOS**
- 下载发布页中主程序 `DMG` 磁盘映像或配套录音工具 `PhonRec` 的 `DMG` 文件。
- 将相应的 `.app` 拖入“应用程序”目录。其中 `PhonRec` 仅限 Apple Silicon 架构。
