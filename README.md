...
本项目在底层深度集成了 **Praat** 的核心算法（通过 `parselmouth` 调用），确保所提取的 F0 数据与在 Praat 软件中手动测量的结果具有**同等的可靠性**。针对元音起始点（VOP）检测，采用了短时能量（STE）与过零率（ZCR）惩罚结合的智能算法。
  ...
  <tr>
    <td width="50%" valign="top">
      ...
      <ul>
        ...
        <li><b>文本匹配</b>：粘贴字表，执行自动 VAD 切分（支持灵活符号拆分）</li>
        <li><b>可视化微调</b>：通过交互界面手动修正边界与提取范围</li>
...
- **可视化支持**：支持导出核密度估计 (KDE) 声调热力图

> 该格式可无缝导入 **Excel** (.xlsx)、**R**、**Python (Pandas)** 以及纯文本 (.txt) 中，用于声学统计分析和传统五度值声调格局图的绘制。
